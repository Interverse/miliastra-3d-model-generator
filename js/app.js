...
export function initApp({ mode = "gia" } = {}) {
  ...
  worker.onmessage = (ev) => {
    ...
    updateOverlay();
    const s = msg.stats;
    const models = Math.ceil(
      msg.decorations.length / MAX_DECORATIONS_PER_MODEL,
    );
    renderStats($("gen-stats"), {
      [s.spritePixels != null ? "Opaque pixels" : "Source triangles"]:
        s.sourceTriangles.toLocaleString(),
      ...(lastParams?.decimate > 0
  ...
  // provided base .gia files: square -> Cuboid (10009001), triangle -> Roof Component
  // (20002125).
  const modelName = (d) => (d.kind === "square" ? "Cuboid" : "Roof Component");
  const modelId = (d) =>
    d.kind === "square" ? SQUARE_MODEL_ID : TRIANGLE_MODEL_ID;
